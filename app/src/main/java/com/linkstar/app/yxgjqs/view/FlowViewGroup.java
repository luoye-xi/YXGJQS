...
public class FlowViewGroup extends ViewGroup
{
	...
	private static final String TAG = "hx/FlowViewGroup";
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			Log.d(TAG, "子View Layout需要的宽高(包含margin)：childWidth:" + childWidth + "   ,childHeight:" + childHeight);
		...
		Log.d(TAG, "系统测量允许的尺寸最大值：widthMeasure:" + widthMeasure + "   ,heightMeasure:" + heightMeasure);
		Log.d(TAG, "经过我们测量实际的尺寸(不包括父控件的padding)：maxLineWidth:" + maxLineWidth + "   ,totalHeight:" + totalHeight);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (curLeft + childWidth > rightLimit)
			{
				...
				viewR = viewL + childW;
				viewB = viewT + childH;
				// child.layout(baseLeft + params.leftMargin, curTop + params.topMargin,
				// baseLeft + params.leftMargin + child.getMeasuredWidth(), curTop +
				// params.topMargin + child.getMeasuredHeight());
				// Log.i(TAG,"新的一行:" +" ,baseLeft:"+baseLeft +" curTop:"+curTop+"
				// baseLeft+childWidth:"+(baseLeft+childWidth)+" curTop+childHeight:"+ (
				// curTop+childHeight));
			...
			} else
			{
				...
				// child.layout(curLeft + params.leftMargin, curTop + params.topMargin, curLeft
				// + params.leftMargin + child.getMeasuredWidth(), curTop + params.topMargin +
				// child.getMeasuredHeight());
				// Log.i(TAG,"当前行:"+changed +" ,curLeft:"+curLeft +" curTop:"+curTop+"
				// curLeft+childWidth:"+(curLeft+childWidth)+"
				// curTop+childHeight:"+(curTop+childHeight));
